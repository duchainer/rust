...
impl IndexBuilder {
    ...
    pub fn build(self) -> Index {
        let IndexBuilder { save, fields, field_tokenizers, ref_field, pipeline, language } = self;

        let index = fields.iter().map(|f| (f.clone(), InvertedIndex::new())).collect();
...
mod tests {
    ...
    fn add_field_to_builder() {
        let idx = IndexBuilder::new().add_fields(&["foo", "bar", "baz"]).build();
